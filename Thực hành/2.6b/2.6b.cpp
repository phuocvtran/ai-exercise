...
public:
    ...
    void BFS(int curr, int goal, vector<int> currPath, vector<vector<int>> &allPaths, int time, int tmpTime, vector<bool> isVisited)
    {
        // Nếu đỉnh đã đi qua
...
int main()
{
    ...
    g.BFS(start, start, vector<int>(), allPaths, 0, 0, vector<bool>(nVertex, false));
